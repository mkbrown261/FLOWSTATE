...
function switchTab(tab) {
  ...
  if (tab==='audio')   initFlowstateAudio();
}
...
let audioState = {
  ...
  startedAt: null,
  project: null,
  tracks: [],               // [{id,name,color,muted,solo,volume,pan,clips:[]}]
  selectedTrackId: null,
  clawflowActive: false,
};

const TRACK_COLORS = ['#a855f7','#ec4899','#3b82f6','#10b981','#f59e0b','#06b6d4','#ef4444','#8b5cf6','#14b8a6','#f97316'];

async function initFlowstateAudio() {
  if (audioState.inited) return;
  audioState.inited = true;

  // Check ClawFlow
  try { const r = await fetch('/api/clawbot/status'); const d = await r.json(); audioState.clawflowActive = d.subscriptionActive; } catch(e){}

  // Wire toolbar
  document.getElementById('aud-play')?.addEventListener('click', audTogglePlay);
  document.getElementById('aud-stop')?.addEventListener('click', audStop);
  document.getElementById('aud-rec')?.addEventListener('click', audToggleRecord);
  document.getElementById('aud-loop')?.addEventListener('click', audToggleLoop);
  document.getElementById('aud-skip-back')?.addEventListener('click', () => { audStop(); audSetTime(0); });
  document.getElementById('aud-add-track')?.addEventListener('click', audAddTrack);
  document.getElementById('aud-new-project')?.addEventListener('click', audNewProject);
  document.getElementById('aud-export')?.addEventListener('click', audExport);
  document.getElementById('aud-ai-level')?.addEventListener('input', (e) => {
    audioState.aiLevel = parseInt(e.target.value);
    const badge = document.getElementById('aud-ai-badge');
    const lvl = audioState.aiLevel;
    if (badge) badge.textContent = lvl === 0 ? 'Manual' : lvl === 100 ? 'Full AI' : `AI ${lvl}%`;
  });
  document.getElementById('aud-bpm')?.addEventListener('blur', (e) => {
    const v = parseInt(e.target.textContent) || 120;
    audioState.bpm = Math.max(40, Math.min(300, v));
    e.target.textContent = audioState.bpm;
    audRenderRuler();
  });
  document.getElementById('aud-key')?.addEventListener('change', (e) => { audioState.key = e.target.value; });

  // AI buttons
  document.getElementById('aud-btn-full-track')?.addEventListener('click', () => audAiGenerate('generate_track'));
  document.getElementById('aud-btn-melody')?.addEventListener('click', () => audAiGenerate('generate_melody'));
  document.getElementById('aud-btn-beat')?.addEventListener('click', () => audAiGenerate('generate_beat'));
  document.getElementById('aud-btn-stems')?.addEventListener('click', () => audAiGenerate('separate_stems'));
  document.getElementById('aud-btn-master')?.addEventListener('click', () => audAiGenerate('master_track'));
  document.getElementById('aud-btn-arrange')?.addEventListener('click', () => audAiGenerate('suggest_arrangement'));
  document.getElementById('aud-btn-analyze')?.addEventListener('click', () => audAiGenerate('detect_key_bpm'));

  // Boot default project
  await audNewProject();
}

async function audNewProject() {
  try {
    const r = await fetch('/api/audio/project/create', {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ name:'Untitled Project', bpm: audioState.bpm, key: audioState.key })
    });
    const d = await r.json();
    if (d.project) {
      audioState.tracks = d.project.tracks.map(t => ({ ...t, clips:[] }));
    }
  } catch(e) {
    // Fallback to local tracks
    audioState.tracks = [0,1,2,3].map((i) => ({
      id: 'track-'+i, name: ['Track 1','Track 2','Drums','Bass'][i],
      color: TRACK_COLORS[i], muted:false, solo:false, volume:0.8, pan:0, clips:[]
    }));
  }
  audStop();
  audRenderAll();
...
function audRenderSidebar() {
  const sidebar = document.getElementById('aud-sidebar');
  if (!sidebar) return;
  sidebar.innerHTML = audioState.tracks.map(t => `
    <div class="track-header" data-tid="${t.id}">
      <div class="track-color-bar" style="background:${t.color}"></div>
      <div style="flex:1;min-width:0">
        <input class="track-name-input" value="${escHtml(t.name)}" onchange="audRenameTrack('${t.id}',this.value)">
        <div style="display:flex;align-items:center;gap:4px;margin-top:4px">
          <button class="track-btn ${t.muted?'muted':''}" onclick="audToggleMute('${t.id}')" title="Mute">M</button>
          <button class="track-btn ${t.solo?'soloed':''}" onclick="audToggleSolo('${t.id}')" title="Solo">S</button>
          <input type="range" class="track-vol" min="0" max="100" value="${Math.round(t.volume*100)}" oninput="audSetVol('${t.id}',this.value)" title="Volume">
        </div>
      </div>
    </div>
  `).join('');
...
function audRenderLanes() {
  const lanes = document.getElementById('aud-lanes');
  if (!lanes) return;
  const beatsVisible = 64;
  const pw = audioState.pixelsPerBeat * beatsVisible;
  lanes.style.width = pw + 'px';
  lanes.innerHTML = audioState.tracks.map(t => `
    <div class="track-lane" data-tid="${t.id}" style="width:${pw}px" onclick="audSelectTrack('${t.id}')" ondblclick="audAddClipAt(event,'${t.id}')">
      ${t.clips.map(clip => `
        <div class="audio-clip" style="left:${clip.startBeat*audioState.pixelsPerBeat}px;width:${clip.durationBeats*audioState.pixelsPerBeat}px;background:${t.color}88;border-color:${t.color}" title="${escHtml(clip.name)}">
          <div class="clip-label">${escHtml(clip.name)}</div>
          <svg class="clip-wave" viewBox="0 0 100 30" preserveAspectRatio="none">
            <polyline points="${generateFakeWaveform(clip.id)}" fill="none" stroke="rgba(255,255,255,0.5)" stroke-width="1"/>
          </svg>
        </div>
      `).join('')}
    </div>
...
function audRenderMixer() {
  const mixer = document.getElementById('aud-mixer');
  if (!mixer) return;
  const allCh = [...audioState.tracks, { id:'master', name:'Master', color:'#fff', muted:false, volume:1, pan:0 }];
  mixer.innerHTML = allCh.map(t => `
    <div class="mixer-channel">
      <div class="mixer-ch-name" style="color:${t.color}">${escHtml(t.name)}</div>
      <div style="display:flex;flex-direction:column;align-items:center;gap:3px">
        <input type="range" class="mixer-pan" min="-100" max="100" value="${Math.round((t.pan||0)*100)}" title="Pan" oninput="audSetPan('${t.id}',this.value)">
        <div class="mixer-fader-wrap">
          <input type="range" class="mixer-fader" min="0" max="100" value="${Math.round(t.volume*100)}" oninput="audSetVol('${t.id}',this.value)" orient="vertical">
        </div>
        <div class="mixer-db">${volToDb(t.volume)}</div>
      </div>
      <button class="mixer-mute ${t.muted?'muted':''}" onclick="audToggleMute('${t.id}')">${t.muted?'MUTED':'M'}</button>
    </div>
  `).join('');
}

function audRenderRuler() {
  const ruler = document.getElementById('aud-ruler');
  if (!ruler) return;
  const beats = 64;
  ruler.style.width = (beats * audioState.pixelsPerBeat) + 'px';
  let html = '';
  for (let b = 0; b <= beats; b += 4) {
    const x = b * audioState.pixelsPerBeat;
    const bar = (b / 4) + 1;
    html += `<div class="ruler-tick" style="left:${x}px"><div class="ruler-label">${bar}</div><div class="ruler-line" style="height:${b%16===0?12:7}px"></div></div>`;
  }
  ruler.innerHTML = html;
}

function audTogglePlay() {
  if (audioState.playing) { audPause(); }
  else { audPlay(); }
}

function audPlay() {
  audioState.playing = true;
  audioState.startedAt = performance.now() - (audioState.currentTime * 1000);
  document.getElementById('aud-play-icon')?.classList.replace('fa-play','fa-pause');
  document.getElementById('aud-play')?.classList.add('play-active');
  audRAF();
}
...
function audPause() {
  ...
  document.getElementById('aud-play-icon')?.classList.replace('fa-pause','fa-play');
  document.getElementById('aud-play')?.classList.remove('play-active');
}
...
function audToggleRecord() {
  audioState.recording = !audioState.recording;
  document.getElementById('aud-rec')?.classList.toggle('active', audioState.recording);
  if (audioState.recording && !audioState.playing) audPlay();
}

function audToggleLoop() {
  audioState.looping = !audioState.looping;
  document.getElementById('aud-loop')?.classList.toggle('play-active', audioState.looping);
}

function audRAF() {
  if (!audioState.playing) return;
  const now = performance.now();
  audioState.currentTime = (now - audioState.startedAt) / 1000;
  const beatsTotal = 64;
  const dur = (beatsTotal / audioState.bpm) * 60;
  if (audioState.looping && audioState.currentTime >= dur) {
    audioState.currentTime = 0;
    audioState.startedAt = now;
  }
  audUpdatePlayhead();
  audUpdateTimeDisplay();
  audioState.rafId = requestAnimationFrame(audRAF);
}

function audUpdatePlayhead() {
  const tl = document.getElementById('aud-timeline');
  if (!tl) return;
  const beatSec = 60 / audioState.bpm;
  const beat = audioState.currentTime / beatSec;
  const x = beat * audioState.pixelsPerBeat;
  let ph = document.getElementById('aud-playhead');
  if (!ph) {
    ph = document.createElement('div'); ph.id = 'aud-playhead'; ph.className = 'playhead';
    ph.innerHTML = '<div class="playhead-arrow"></div>';
    tl.appendChild(ph);
  }
  ph.style.left = x + 'px';
}

function audUpdateTimeDisplay() {
  const el = document.getElementById('aud-time');
  if (!el) return;
  const t = audioState.currentTime;
  const m = Math.floor(t / 60);
  const s = Math.floor(t % 60);
  const ms = Math.floor((t % 1) * 1000);
  el.textContent = `${m}:${String(s).padStart(2,'0')}.${String(ms).padStart(3,'0')}`;
}

function audSetTime(sec) {
  audioState.currentTime = sec;
  audUpdatePlayhead();
  audUpdateTimeDisplay();
}

function audSelectTrack(id) { audioState.selectedTrackId = id; }

function audAddTrack() {
  const idx = audioState.tracks.length;
  audioState.tracks.push({
    id: 'track-' + Date.now(), name: 'Track ' + (idx + 1),
    color: TRACK_COLORS[idx % TRACK_COLORS.length],
    muted:false, solo:false, volume:0.8, pan:0, clips:[]
  });
  audRenderAll();
}

function audRenameTrack(id, name) {
  const t = audioState.tracks.find(t=>t.id===id);
  if (t) { t.name = name; audRenderMixer(); }
}

function audToggleMute(id) {
  const t = audioState.tracks.find(t=>t.id===id);
  if (t) { t.muted = !t.muted; audRenderSidebar(); audRenderMixer(); }
}

function audToggleSolo(id) {
  const t = audioState.tracks.find(t=>t.id===id);
  if (t) { t.solo = !t.solo; audRenderSidebar(); }
}

function audSetVol(id, val) {
  const t = audioState.tracks.find(t=>t.id===id);
  if (t) { t.volume = parseInt(val)/100; audRenderMixer(); }
}

function audSetPan(id, val) {
  const t = audioState.tracks.find(t=>t.id===id);
  if (t) t.pan = parseInt(val)/100;
}

function audAddClipAt(e, trackId) {
  const lane = e.currentTarget;
  const rect = lane.getBoundingClientRect();
  const clickX = e.clientX - rect.left;
  const beat = Math.round(clickX / audioState.pixelsPerBeat);
  const t = audioState.tracks.find(t=>t.id===trackId);
  if (!t) return;
  t.clips.push({
    id: 'clip-'+Date.now(), trackId, startBeat: beat, durationBeats: 8,
    name: 'Clip '+(t.clips.length+1), aiGenerated: false,
  });
  audRenderLanes();
}

async function audAiGenerate(tool) {
  const prompt  = document.getElementById('aud-ai-prompt')?.value || 'Create something great';
  const style   = document.getElementById('aud-ai-style')?.value || 'pop';
  const dur     = parseInt(document.getElementById('aud-ai-dur')?.value || '30');
  const results = document.getElementById('aud-ai-results');
  if (!results) return;

  // Lock check
  const lockedTools = ['generate_track','generate_melody','generate_beat','separate_stems','master_track','pitch_correct','suggest_arrangement'];
  if (lockedTools.includes(tool) && !audioState.clawflowActive) {
    results.innerHTML = `<div class="audio-result-clip" style="border-color:rgba(245,158,11,.3)"><i class="fas fa-lock" style="color:var(--warn)"></i><div><strong style="color:var(--warn)">ClawFlow Required</strong><br><span style="font-size:11px;color:var(--text-m)">Subscribe to ClawFlow ($20 first month) to unlock AI music generation.</span></div></div>`;
    return;
  }

  results.innerHTML = `<div class="audio-result-clip"><i class="fas fa-spinner fa-spin" style="color:#10b981"></i><span>Generating — this may take up to 60s…</span></div>`;

  try {
    const res = await fetch('/api/audio/generate', {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ tool, prompt, style, bpm: audioState.bpm, key: audioState.key, durationSeconds: dur })
    });
    const data = await res.json();

    if (data.error === 'clawflow_required') {
      results.innerHTML = `<div class="audio-result-clip" style="border-color:rgba(245,158,11,.3)"><i class="fas fa-lock" style="color:var(--warn)"></i><div><strong style="color:var(--warn)">ClawFlow Required</strong><br><span style="font-size:11px;color:var(--text-m)">${escHtml(data.result?.message||'Subscribe to ClawFlow to unlock.')}</span></div></div>`;
      return;
    }
    ...
    if (data.suggestion) {
      ...
      results.innerHTML = `
        <div class="audio-result-clip" style="flex-direction:column;align-items:flex-start;gap:6px">
          <strong style="color:#10b981">&#127926; Arrangement Suggestion</strong>
          <div style="font-size:11px;color:var(--text-m)"><strong>Chord Progression:</strong> ${escHtml(s.chordProgression)}</div>
          <div style="font-size:11px;color:var(--text-m)"><strong>Structure:</strong> ${s.arrangement.map(escHtml).join(' → ')}</div>
          <div style="font-size:11px;color:var(--text-m)"><strong>Tracks:</strong> ${s.suggestedTracks.map(escHtml).join(', ')}</div>
          <div style="font-size:11px;color:var(--text-m)"><strong>Tips:</strong> ${s.productionTips.map(escHtml).join(' · ')}</div>
        </div>`;
      return;
    }

    if (data.bpm && tool === 'detect_key_bpm') {
      results.innerHTML = `
        <div class="audio-result-clip" style="flex-direction:column;align-items:flex-start;gap:4px">
          <strong style="color:#10b981">&#127926; Analysis Complete</strong>
          <div style="font-size:11px"><strong>BPM:</strong> ${data.bpm} &nbsp; <strong>Key:</strong> ${escHtml(data.key)}</div>
          <div style="font-size:11px"><strong>Loudness:</strong> ${data.loudnessLufs?.toFixed(1)} LUFS &nbsp; <strong>Peak:</strong> ${data.peakDb?.toFixed(1)} dBFS</div>
          ${(data.suggestions||[]).map(s=>`<div style="font-size:10px;color:var(--text-m)">&#x2022; ${escHtml(s)}</div>`).join('')}
        </div>`;
      // Apply detected BPM/key
      const bpmEl = document.getElementById('aud-bpm');
      const keyEl = document.getElementById('aud-key');
      if (bpmEl) { audioState.bpm = data.bpm; bpmEl.textContent = data.bpm; audRenderRuler(); }
      if (keyEl) { audioState.key = data.key; keyEl.value = data.key; }
      return;
    }

    if (data.audioUrl) {
      // Real audio returned — add to timeline as a clip
      const track = audioState.tracks.find(t => !t.clips.length) || audioState.tracks[0];
      if (track) {
        track.clips.push({
          id:'clip-ai-'+Date.now(), trackId: track.id,
          startBeat: 0, durationBeats: Math.round((dur/60)*audioState.bpm),
          name: '🎵 AI: '+prompt.slice(0,20), aiGenerated:true, sourceUrl: data.audioUrl,
        });
        audRenderLanes();
      }
      results.innerHTML = `<div class="audio-result-clip"><i class="fas fa-check" style="color:#10b981"></i><div><strong>Generated!</strong> Added to timeline.<br><audio controls src="${data.audioUrl}" style="width:100%;margin-top:4px;height:28px"></audio></div></div>`;
    } else if (data.predictionId) {
      // Poll Replicate
      results.innerHTML = `<div class="audio-result-clip"><i class="fas fa-spinner fa-spin" style="color:#10b981"></i><span>Generating with MusicGen… polling for result.</span></div>`;
      audPollReplicate(data.predictionId, prompt, dur, results);
    } else {
      // Demo / queued
      results.innerHTML = `<div class="audio-result-clip"><i class="fas fa-circle-info" style="color:#10b981"></i><div style="font-size:12px">${escHtml(data.message||data.result?.message||'Queued.')}</div></div>`;
    }
  } catch(e) {
    results.innerHTML = `<div class="audio-result-clip" style="border-color:rgba(239,68,68,.3)"><i class="fas fa-triangle-exclamation" style="color:var(--danger)"></i><span>Error — check your network or API keys.</span></div>`;
  }
}

async function audPollReplicate(predId, prompt, dur, resultsEl) {
  for (let i = 0; i < 30; i++) {
    await new Promise(r => setTimeout(r, 3000));
    try {
      const r = await fetch('/api/audio/generate/poll/' + predId);
      const d = await r.json();
      if (d.status === 'succeeded' && d.audioUrl) {
        const track = audioState.tracks.find(t => !t.clips.length) || audioState.tracks[0];
        if (track) {
          track.clips.push({ id:'clip-ai-'+Date.now(), trackId:track.id, startBeat:0, durationBeats:Math.round((dur/60)*audioState.bpm), name:'🎵 AI: '+prompt.slice(0,20), aiGenerated:true, sourceUrl:d.audioUrl });
          audRenderLanes();
        }
        if (resultsEl) resultsEl.innerHTML = `<div class="audio-result-clip"><i class="fas fa-check" style="color:#10b981"></i><div><strong>Generated!</strong><br><audio controls src="${d.audioUrl}" style="width:100%;margin-top:4px;height:28px"></audio></div></div>`;
        return;
      }
      if (d.status === 'failed') {
        if (resultsEl) resultsEl.innerHTML = `<div class="audio-result-clip" style="border-color:rgba(239,68,68,.3)"><i class="fas fa-xmark" style="color:var(--danger)"></i><span>Generation failed: ${escHtml(d.error||'unknown error')}</span></div>`;
        return;
      }
    } catch(e) {}
  }
  if (resultsEl) resultsEl.innerHTML = `<div class="audio-result-clip"><i class="fas fa-hourglass" style="color:var(--warn)"></i><span>Timed out — the generation may still complete. Try again.</span></div>`;
}

function audExport() {
  notify('Export: In the full local app, this renders a 44.1kHz stereo WAV/MP3. Currently running in Hub preview mode.','info');
}

function generateFakeWaveform(seed) {
  let h = 0;
  for (let i=0;i<seed.length;i++) h=(h*31+seed.charCodeAt(i))&0xffff;
  const pts = [];
  for (let i=0;i<=100;i+=2) {
    h = (h*1103515245+12345)&0x7fffffff;
    const y = 15 + (h%16) * ((i%10===0)?1.2:0.8) * (i<5||i>95?0.3:1);
    pts.push(i+','+y);
  }
  return pts.join(' ');
}
